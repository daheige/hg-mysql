...
class mysqlPdo
{
    ...
    private function __construct($config = [])
    {
        ...
        $this->db_pwd     = isset($config['db_pwd']) ? $config['db_pwd'] : '1234';
        ...
        try {
            ...
            $this->dbh = new PDO($this->dsn, $this->db_user, $this->db_pwd);
            $this->dbh->exec('SET NAMES ' . $this->db_charset);
    ...
    public static function getInstance($config = [])
    {
        ...
        if (!isset(self::$instances[$config_id]) || !self::$instances[$config_id] instanceof self) {
            $class                       = get_called_class();
            self::$instances[$config_id] = new $class($config);
        }
     ...
     * @param  String  $queryMode 查询方式(All or one)
     * @param  Boolean $debug
     * @return Array
     */
    ...
    {
        return $this->query($strSql, 'one', $debug);
    }

    ////获取多个记录
    public function getAll($strSql, $debug = false)
    {
        return $this->query($strSql, 'all', $debug);
    }
